...
export const ReceptionView = () => {
  ...
  const handleAssignRFID = (orderId: string) => {
    ...
    const rfidTagId = `RFID-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    
    // Update order with RFID tag ID
    updateOrder(orderId, {
      rfidTagId: rfidTagId,
    ...
    toast.success(`RFID Tag ${rfidTagId} berhasil ditetapkan ke order`);
  };
