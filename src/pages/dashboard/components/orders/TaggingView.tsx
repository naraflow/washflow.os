...
import { Radio, Eye, AlertCircle, Clock, CheckCircle2 } from "lucide-react";

export const TaggingView = () => {
  const orders = useDashboardStore((state) => state.orders);
  const [selectedOrder, setSelectedOrder] = useState<Order | null>(null);
  const [isTaggingModalOpen, setIsTaggingModalOpen] = useState(false);
  const [isDetailsModalOpen, setIsDetailsModalOpen] = useState(false);

  // Filter orders that need tagging
  // Include orders that:
  // 1. Have taggingRequired = true AND taggingStatus = 'pending', OR
  // 2. Don't have taggingStatus set yet (backward compatibility for old orders)
  const pendingTaggingOrders = orders.filter(
    (order) => {
      // New orders created by kasir should have taggingRequired = true and taggingStatus = 'pending'
      if (order.taggingRequired && order.taggingStatus === 'pending') {
        return true;
      }
      // Backward compatibility: if order doesn't have taggingStatus but is in reception stage
      // and doesn't have RFID tag yet, it needs tagging
  ...
  // Calculate urgency (time since created)
